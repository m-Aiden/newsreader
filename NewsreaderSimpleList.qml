...
Item {
	...
	QtObject {
		...
		property NewsreaderThreeStateButton prvButUp: butUp
		property NewsreaderThreeStateButton prvButDown: butDown
		...
		function updateButtons() {
			...
			//had to do manual setting of visible property of buttons here because there is a problem with icon in NewsreaderThreeStateButton when directly bound
		...
		function updateScrollbar() {
			...
			scrollbar.height = Math.floor(newsreaderMarkup.height / pageCount);
			...
			topMargin = (topMargin + scrollbar.height) >= newsreaderMarkup.height ? newsreaderMarkup.height - scrollbar.height - 1 : topMargin;
	...
	Rectangle {
		...
		Column {
			id: repeaterColumn
			width: newsreaderMarkup.width
			height: newsreaderMarkup.height
			anchors {
				top: newsreaderMarkup.top
				topMargin: 6
				left: newsreaderMarkup.left
				leftMargin: 6
			}
			spacing: 5
	...
	NewsreaderThreeStateButton {
		id: butDown
		width: 38
		height: newsreaderMarkup.height / 2
		...
		anchors {
			bottom: newsreaderMarkup.bottom
			left: newsreaderMarkup.right
